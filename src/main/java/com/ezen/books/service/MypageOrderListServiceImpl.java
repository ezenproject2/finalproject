...
import com.ezen.books.repository.MypageOrderListMapper;
...
public class MypageOrderListServiceImpl implements MypageOrderListService {
    ...
    private MypageOrderListMapper mypageOrderListMapper;
    private String iamportApiKey;
    private String iamportApiSecret;

    @Autowired
    public MypageOrderListServiceImpl(
            MypageOrderListMapper mypageOrderListMapper,
            @Value("${iamport_rest_api_key}") String iamportApiKey,
            @Value("${iamport_rest_api_secret}") String iamportApiSecret) {
        this.mypageOrderListMapper = mypageOrderListMapper;
    ...
    public List<OrdersVO> getAllOrderList(long mno) {
        return mypageOrderListMapper.getAllOrderList(mno);
    }

    @Override
    public List<OrderDetailVO> getOrderDetailList(String orno) {
        return mypageOrderListMapper.getOrderDetailList(orno);
    }
    ...
    public boolean isOrderEmpty(long mno) {
        ...
        List<String> ornoList = mypageOrderListMapper.getOrnoList(mno);
        return ornoList.isEmpty();
    }

    @Override
    public MemberVO getMember(long mno) {
        return mypageOrderListMapper.getMember(mno);
    }

    @Override
    public GradeVO getMemberGrade(Long gno) {
        return mypageOrderListMapper.getMemberGrade(gno);
    }

    @Override
    public OrdersVO getOrder(String orno) {
        return mypageOrderListMapper.getOrder(orno);
    }
    ...
    public DeliveryVO getDelivery(String orno) {
        return mypageOrderListMapper.getDelivery(orno);
    }

    @Override
    public PickUpVO getPickUp(String orno) {
        return mypageOrderListMapper.getPickUp(orno);
    }

    @Override
    public OfflineStoreVO getOfflineStore(long osno) {
        return mypageOrderListMapper.getOfflineStore(osno);
    }

    @Override
    public PaymentVO getPayment(String orno) {
        return mypageOrderListMapper.getPayment(orno);
    }
    ...
    public int refundWithIamport(long odno, String orno, int qtyPrice) throws IOException, URISyntaxException, InterruptedException {
        IamportAccessToken iamportToken = MypageOrderListService.super.issueIamportToken(iamportApiKey, iamportApiSecret);
        ...
        String impUid = mypageOrderListMapper.getImpUid(orno);
        // NOTE: 화면에서 가져올 수도 있음.
        String reason = "단순 변심";
...
//        log.info("The response body: {}", result.getBody());
        ...
        int isDone = mypageOrderListMapper.applyRefundToOrderDetail(odno, orno);
    ...
    private List<OrderDetailProductDTO> generateOrderDetailProductList(String orno) {
        ...
        List<Long> odnoList = mypageOrderListMapper.getOdnoList(orno);

        for(Long odno : odnoList) {
            OrderDetailProductDTO detailProduct = new OrderDetailProductDTO();

            OrderDetailVO orderDetail = mypageOrderListMapper.findOrderDetail(odno);
            ProductVO product = mypageOrderListMapper.getProduct(orderDetail.getPrno());
