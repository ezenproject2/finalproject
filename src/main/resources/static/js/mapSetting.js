    ...
    function moveToStore(name, lat, lon) {
        ...
        getStoreInfoFromServer(name).then(result => {
            if (result != null) {
                ...
                div.innerHTML = "";
                let str = `<h2>${result.name}</h2><p><span class="criterion">매장주소</span>`;
                str += `<span>${result.address}</span></p><p><span class="criterion">영업시간</span>`;
                str += `<span>${result.hours}</span></p><p><span class="criterion">휴점</span>`;
                str += `<span>${result.holiday}</span></p>`;
                str += `<a href="tel:${result.phone}" class="tel"><i class="ic ic_call"></i><span>매장문의</span><span>${result.phone}</span></a>`;
                div.innerHTML = str;
            }
        });
    }
